				...
				<form name="form" id="form" role="form" method="post"
					...
					<div class="mb-3">
						<label for="title">사진 등록</label> <br> <img id="preview"
							src="resources/upload/user.svg" width="300" alt="로컬에 있는 이미지가 보여지는 영역">
		...
		$('#datepicker').datepicker({
				...
				success : function(data) {
					...
					$.each(data, function(i){
						console.log(i + " / " + data[i])
						 $("#selectBox").append("<option value='"+data[i].job+"'>"+data[i].job+"</option>")
					});  
				},
			...
			$.ajax({
				...
				success : function(data) {
					$.each(data, function(i){
						console.log(i + " / " + data[i])
						 $("#selectBox2").append("<option value='"+data[i].deptno+"'>"+data[i].deptno+"</option>")
					});  
				},
				 error:function(request,status,error){
					    console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
